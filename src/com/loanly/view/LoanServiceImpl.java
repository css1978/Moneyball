...
public class LoanServiceImpl implements LoanService{
     ...
     */
    public PageBean queryForPage(int pageSize,int page,Date loanStartDate,Date loanEndDate,String platform_id,String status){
...
*/
    	int allRow = loanDao.getAllRowCount(user_name,loanStartDate,loanEndDate,platform_id,status);
        int totalPage = PageBean.countTotalPage(pageSize, allRow);    //总页数
        final int offset = PageBean.countOffset(pageSize, page);    //当前页开始记录
        final int length = pageSize;    //每页记录数
        final int currentPage = PageBean.countCurrentPage(page);
        //hql += " order by expire_date desc";
        List<Loan> list = loanDao.queryForPage(user_name,loanStartDate,loanEndDate,platform_id,status,offset, length);        //"一页"的记录
        //    //总记录数
